...
contract WithdrawalDelayer is ReentrancyGuard, IWithdrawalDelayer {
    ...
    address private _wyzthGovernance; // Governance who control the system parameters
    ...
    address public wyzthRollupAddress; // wyzth Rollup Address who can send funds to this smart contract
    ...
    event NewWyzthGovernanceAddress(address newWyzthGovernanceAddress);

    // Event emitted when the contract is initialized
    event InitializeWithdrawalDelayerEvent(
        uint64 initialWithdrawalDelay,
        address initialWyzthGovernanceAddress,
     ...
     * @param _initialWyzthRollup Smart contract responsible of making deposits and it's able to change the delay
     * @param _initialWyzthGovernanceAddress can claim the funds in an emergency mode
    ...
    constructor(
        ...
        address _initialWyzthRollup,
        address _initialWyzthGovernanceAddress,
        address payable _initialEmergencyCouncil
    ) public {
        require(
            _initialWyzthRollup != address(0),
            "WithdrawalDelayer::withdrawalDelayerInitializer ADDRESS_0_NOT_VALID"
        );

        _withdrawalDelay = _initialWithdrawalDelay;
        wyzthRollupAddress = _initialWyzthRollup;
        _wyzthGovernance = _initialWyzthGovernanceAddress;
        _emergencyCouncil = _initialEmergencyCouncil;
        _emergencyMode = false;

        emit InitializeWithdrawalDelayerEvent(
            _initialWithdrawalDelay,
            _initialWyzthGovernanceAddress,
            _initialEmergencyCouncil
        );
    }

    /**
     * @notice Getter of the current `_wyzthGovernance`
     * @return The `_wyzthGovernance` value
     */
    function getWyzthGovernanceAddress()
        external
        override
        view
        returns (address)
    {
        return _wyzthGovernance;
    }
    ...
    function claimGovernance() public override {
        ...
        _wyzthGovernance = pendingGovernance;
        pendingGovernance = address(0);
        emit NewWyzthGovernanceAddress(_wyzthGovernance);
    }
    ...
    function deposit(
        ...
        if (msg.value != 0) {
            ...
            require(
                IERC20(_token).allowance(wyzthRollupAddress, address(this)) >=
                    _amount,
    ...
    function escapeHatchWithdrawal(
        ...
        require(
            msg.sender == _emergencyCouncil || msg.sender == _wyzthGovernance,
            "WithdrawalDelayer::escapeHatchWithdrawal: ONLY_GOVERNANCE"
        );
        if (
            msg.sender == _emergencyCouncil &&
            _emergencyCouncil != _wyzthGovernance
        ) {
            require(
                uint64(now) >=
