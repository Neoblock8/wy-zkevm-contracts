...
contract WyzthAuctionProtocol is Initializable, ReentrancyGuardUpgradeable,IWyzthAuctionProtocol {
    ...
    // Wyzth Network Token with which the bids will be made
    IHEZToken public tokenHEZ;
    // WyzthRollup smartcontract address
    address public wyzthRollup;
    // Wyzth Governance smartcontract address who controls some parameters and collects HEZ fee
    ...
    event InitializeWyzthAuctionProtocolEvent(
    ...
    modifier onlyGovernance() {
        require(
            ...
            "WyzthAuctionProtocol::onlyGovernance: ONLY_GOVERNANCE"
        );
        _;
    }
     ...
     * @param token Wyzth Network token with which the bids will be made
     * @param wyzthRollupAddress address authorized to forge
     * @param donationAddress address that can claim donated tokens
     * @param _governanceAddress Wyzth Governance smartcontract
     * @param bootCoordinatorAddress Boot Coordinator Address
     */
    function wyzthAuctionProtocolInitializer(
        address token,
        uint128 genesis,
        address wyzthRollupAddress,
        ...
        require(
            wyzthRollupAddress != address(0),
            "WyzthAuctionProtocol::wyzthAuctionProtocolInitializer ADDRESS_0_NOT_VALID"
        );
        ...
        require(
            ...
            "WyzthAuctionProtocol::wyzthAuctionProtocolInitializer GENESIS_BELOW_MINIMAL"
        );

        tokenHEZ = IHEZToken(token);

        genesisBlock = genesis;
        wyzthRollup = wyzthRollupAddress;
        governanceAddress = _governanceAddress;
        _donationAddress = donationAddress;
        _bootCoordinator = bootCoordinatorAddress;
        bootCoordinatorURL = _bootCoordinatorURL;

        emit InitializeWyzthAuctionProtocolEvent(
            donationAddress,
        ...
        onlyGovernance
    {
        require(
            ...
            "WyzthAuctionProtocol::setSlotDeadline: GREATER_THAN_BLOCKS_PER_SLOT"
        );
        ...
        onlyGovernance
    {
        require(
            ...
            "WyzthAuctionProtocol::setOutbidding: OUTBIDDING_NOT_VALID"
        );
        ...
        onlyGovernance
    {
        require(
            newAllocationRatio[0] <= 10000 &&
                ...
                10000,
            "WyzthAuctionProtocol::setAllocationRatio: ALLOCATION_RATIO_NOT_VALID"
        );
        ...
        onlyGovernance
    {
        require(
            ...
            "WyzthAuctionProtocol::setDonationAddress: NOT_VALID_ADDRESS"
        );
        ...
        onlyGovernance
    {
        require(
            ...
            "WyzthAuctionProtocol::changeDefaultSlotSetBid: NOT_VALID_SLOT_SET"
        );
        require(
            _defaultSlotSetBid[slotSet] != 0,
            "WyzthAuctionProtocol::changeDefaultSlotSetBid: SLOT_DECENTRALIZED"
        );
        ...
        override
    {
        require(
            ...
            "WyzthAuctionProtocol::setCoordinator: NOT_VALID_URL"
        );
    ...
    function getMinBidBySlot(uint128 slot) public view returns (uint128) {
        require(
            ...
            "WyzthAuctionProtocol::getMinBidBySlot: AUCTION_CLOSED"
        );
    ...
    function processBid(
        ...
        require(
            ...
            "WyzthAuctionProtocol::processBid: COORDINATOR_NOT_REGISTERED"
        );
        require(
            slot > (getCurrentSlotNumber() + _closedAuctionSlots),
            "WyzthAuctionProtocol::processBid: AUCTION_CLOSED"
        );
        require(
            bidAmount >= getMinBidBySlot(slot),
            "WyzthAuctionProtocol::processBid: BELOW_MINIMUM"
        );
        ...
        require(
            slot <=
                (getCurrentSlotNumber() +
                    _closedAuctionSlots +
                    _openAuctionSlots),
            "WyzthAuctionProtocol::processBid: AUCTION_NOT_OPEN"
        );
        ...
        require(
            ...
            "WyzthAuctionProtocol::processBid: TOKEN_TRANSFER_FAILED"
        );
        pendingBalances[msg.sender] = pendingBalances[msg.sender].add(amount);

        require(
            pendingBalances[msg.sender] >= bidAmount,
            "WyzthAuctionProtocol::processBid: NOT_ENOUGH_BALANCE"
        );
    ...
    function processMultiBid(
        ...
        require(
            ...
            "WyzthAuctionProtocol::processMultiBid AUCTION_CLOSED"
        );
        require(
            endingSlot <=
                (getCurrentSlotNumber() +
                    _closedAuctionSlots +
                    _openAuctionSlots),
            "WyzthAuctionProtocol::processMultiBid AUCTION_NOT_OPEN"
        );
        require(
            maxBid >= minBid,
            "WyzthAuctionProtocol::processMultiBid MAXBID_GREATER_THAN_MINBID"
        );
        // To avoid possible mistakes we don't allow anyone to bid without setting a forger
        require(
            coordinators[msg.sender].forger != address(0),
            "WyzthAuctionProtocol::processMultiBid COORDINATOR_NOT_REGISTERED"
        );
        ...
        require(
            tokenHEZ.transferFrom(msg.sender, address(this), amount),
            "WyzthAuctionProtocol::processMultiBid: TOKEN_TRANSFER_FAILED"
        );
        pendingBalances[msg.sender] = pendingBalances[msg.sender].add(amount);
        ...
        for (uint128 slot = startingSlot; slot <= endingSlot; slot++) {
            ...
            if (slotSets[getSlotSet(slot)]) {
                require(
                    pendingBalances[msg.sender] >= bidAmount,
                    "WyzthAuctionProtocol::processMultiBid NOT_ENOUGH_BALANCE"
                );
                _doBid(slot, bidAmount, msg.sender);
            }
        }
    ...
    function _permit(uint256 _amount, bytes calldata _permitData) internal {
        ...
        require(
            owner == msg.sender,
            "WyzthAuctionProtocol::_permit: OWNER_NOT_EQUAL_SENDER"
        );
        require(
            spender == address(this),
            "WyzthAuctionProtocol::_permit: SPENDER_NOT_EQUAL_THIS"
        );
        require(
            value == _amount,
            "WyzthAuctionProtocol::_permit: WRONG_AMOUNT"
        );
    ...
    function _doBid(
        ...
        require(
            ...
            "WyzthAuctionProtocol::_doBid: BID_MUST_BE_HIGHER"
        );
        ...
        returns (bool)
    {
        require(
            blockNumber < 2**128,
            "WyzthAuctionProtocol::canForge WRONG_BLOCKNUMBER"
        );
        require(
            blockNumber >= genesisBlock,
            "WyzthAuctionProtocol::canForge AUCTION_NOT_STARTED"
        );
    ...
    function forge(address forger) external override {
        require(
            msg.sender == wyzthRollup,
            "WyzthAuctionProtocol::forge: ONLY_WYZTH_ROLLUP"
        );
        require(
            _canForge(forger, block.number),
            "WyzthAuctionProtocol::forge: CANNOT_FORGE"
        );
            ...
            if (slots[slotToForge].bidAmount != 0) {
                ...
                if (slots[slotToForge].bidAmount < minBid) {
                    ...
                    require(
                        ...
                        "WyzthAuctionProtocol::forge: TOKEN_BURN_FAILED"
                    );
    ...
    function claimPendingHEZ(uint128 slot) public {
        require(
            ...
            "WyzthAuctionProtocol::claimPendingHEZ: ONLY_IF_PREVIOUS_SLOT"
        );
        require(
            !slots[slot].fulfilled,
            "WyzthAuctionProtocol::claimPendingHEZ: ONLY_IF_NOT_FULFILLED"
        );
        // If the closedMinBid is 0 it means that we have to take as minBid the one that is set for this slot set,
        // otherwise the one that has been saved will be used
    ...
    function claimHEZ() public nonReentrant {
        ...
        require(
            pending > 0,
            "WyzthAuctionProtocol::claimHEZ: NOT_ENOUGH_BALANCE"
        );
        pendingBalances[msg.sender] = 0;
        require(
            tokenHEZ.transfer(msg.sender, pending),
            "WyzthAuctionProtocol::claimHEZ: TOKEN_TRANSFER_FAILED"
        );
